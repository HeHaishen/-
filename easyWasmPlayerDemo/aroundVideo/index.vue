 ...
 * @LastEditTime: 2022-01-20 16:42:50
-->
<template>
  <div>
  ...
  methods: {
    ...
    initPlayerVideos(arr) {
      arr.forEach((i, k) => {
        ...
        /* 通过easyGbs播放器播放 */
        let eIds = `more-video-${i.id}`;
        let playerVideo = new WasmPlayer(null, eIds, () => {});
        playerVideo.play(i.playUrlMap.FLV, 1);
        this.videoPlayerArr.push(playerVideo);
      });
    },
    // 销毁断流
    ...
    onMore() {
      ...
      window.open(
        `#/moreVideos?setHeadTitle=true&backRouter=${this.$route.fullPath}`
      );
    },
    openVideoWall(e) {
